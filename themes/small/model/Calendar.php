...
class QCalendarSmall extends QCalendarBase {
				...
				foreach ($this->links as $val) {
					if (($val['day'] == $this->cell[$i][$j]['value']) && (($val['month'] == $this->month) || ($val['month'] == '*')) && (($val['year'] == $this->year) || ($val['year'] == '*'))) {
						switch ($val['link']) {		
							...
							case 'div':
								$this->cell[$i][$j]['day'] = "<a href=\"javascript:;\" onclick=\"qCalendarAllDetails('$this->divLongDesc', '$this->theme', '{$this->cell[$i][$j]['value']}', '{$this->month}', '{$this->year}', '{$this->cat_id}')\" title=\"{$val['short_desc']}\">{$this->cell[$i][$j]['value']}</a>";
